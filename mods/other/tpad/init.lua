...
tpad.settings_file = minetest.get_worldpath() ..
"/mod_storage/" .. tpad.mod_name .. ".custom.conf"
...
function tpad.on_place(itemstack, placer, pointed_thing)
    ...
    if placed and placed.name == tpad.nodename then
        ...
        meta:set_string("infotext",
            "TPAD Station by " .. playername .. " - right click to interact")
...
function submit.save(form)
    ...
    if strpadtype == GLOBAL_PAD_STRING and tpad.max_global_pads_reached(form.playername) then
        notify.warn(form.playername,
            "Can't add more pads to the Global Network, set to 'Public' instead")
...
function submit.delete(form)
    minetest.after(0, function()
        ...
        if minetest.pos_to_string(delete_pad.pos) == minetest.pos_to_string(form.clicked_pos) then
            notify.warn(form.playername,
                "You can't delete the current pad, destroy it manually")
            return
        end

        local function reshow_main()
            minetest.after(0, function()
                tpad.on_rightclick(form.clicked_pos, form.node,
                    minetest.get_player_by_name(form.playername))
            end)
        end
...
function tpad.on_rightclick(clicked_pos, node, clicker)
    ...
    if ownername == playername or minetest.get_player_privs(playername).tpad_admin then
        ...
        padname_field:setLabel("This pad name (owned by " ..
        OWNER_ESCAPE_COLOR .. ownername .. WHITE_ESCAPE .. ")")
        ...
        form.state:get("padtype_dropdown"):setSelectedItem(padtype_flag_to_string
        [pad.type])
    elseif pad.type == PRIVATE_PAD then
        ...
        form.state:get("visitor_label"):setText("Pad \"" ..
        pad.name .. "\", owned by " .. OWNER_ESCAPE_COLOR .. ownername)
    end
...
local function forms_add_padlist(state, is_global)
    ...
    if is_global then
        ...
        local global_button = state:button(1.8, 7, 2.5, 0, "global_button",
            "Global Network")
...
tpad.forms.confirm_pad_deletion = smartfs.create("tpad.forms.confirm_pad_deletion",
    function(state)
        state:size(8, 2.5)
...
tpad.forms.global_network_admin = smartfs.create("tpad.forms.global_network_admin",
    function(state)
        state:size(8, 8)
        state:label(0.2, 1, "visitor_label", "Pick a pad from the Global Pads Network")
...
function tpad.decorate_pad_data(pos, pad, ownername)
    ...
    " " .. pad.strpos .. " " .. short_padtype_string[pad.type]
